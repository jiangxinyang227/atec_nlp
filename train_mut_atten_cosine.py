...
from mut_atten_cosine import MutAttenCosine
...
flags.DEFINE_list("hidden_sizes", [256], "Number of hidden units in each layer")
...
flags.DEFINE_string("model_name", "my_model", "File name used for model checkpoints")

flags.DEFINE_float("ratio", 0.1, "eval data ratio")

dataSet = DataSet(filename=FLAGS.data_path, embedding_size=FLAGS.embedding_size, is_concat=False)
...
with tf.Session(config=config) as sess:
    model = MutAttenCosine(embedding_size=FLAGS.embedding_size, hidden_sizes=FLAGS.hidden_sizes,
